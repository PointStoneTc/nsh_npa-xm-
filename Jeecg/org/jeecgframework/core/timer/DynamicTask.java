...
public class DynamicTask {
    ...
    private boolean startTask(TSTimeTaskEntity task) {
        try {
            ...
            // CronTrigger cronTrigger = new CronTrigger("cron_" + task.getId(),Scheduler.DEFAULT_GROUP,
    ...
    public boolean updateCronExpression(TSTimeTaskEntity task) {
        try {
            ...
            if ("1".equals(task.getIsStart())) {
                ...
                // CronTriggerBean trigger = (CronTriggerBean)schedulerFactory.getTrigger("cron_" + task.getId(),
                ...
                if (isSuccess) {
                    /*
                     * task.setIsEffect("1"); task.setIsStart("1"); timeTaskService.updateEntitie(task);
                     */
     ...
     * (CronTriggerBean)schedulerFactory.getTrigger("cron_" + task.getId(), Scheduler.DEFAULT_GROUP);
     * String originExpression = trigger.getCronExpression(); //检查运行中的任务触发规则是否与新规则一致 if
     * (!originExpression.equalsIgnoreCase(newExpression)) { trigger.setCronExpression(newExpression);
     * schedulerFactory.rescheduleJob("cron_" + task.getId(), Scheduler.DEFAULT_GROUP, trigger); }
     * }else{ //立即生效 startTask(task); task.setIsEffect("1"); task.setIsStart("1");
     * timeTaskService.updateEntitie(task); systemService.addLog(("立即生效开启任务")+task.getTaskId(),
     * Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
     * logger.info(("立即生效开启任务")+"-------TaskId:"+task.getTaskId()+"-------Describe:"+task.
     * getTaskDescribe()+"-----ClassName:"+task.getClassName() ); }
     * 
     * 
     * return true; } catch (SchedulerException e) {
     * logger.error("updateCronExpression SchedulerException" + " cron_" + task.getId() +
     * e.getMessage()); } catch (ParseException e) { logger.error("updateCronExpression ParseException"
     * + " cron_" + task.getId() + e.getMessage()); }
     * 
     * return false; }
     */
    ...
    private void scheduleJob(TSTimeTaskEntity task) throws SchedulerException {
        ...
        // build 任务触发器
        CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("cron_" + task.getId()).withSchedule(cronScheduleBuilder)// 标明触发时间
                .build();
        // 向调度器注册 定时任务
