...
public class SimpleJdbcTemplate extends JdbcTemplate {
    ...
    public long findForLong(final String sql, Map parameters) {
        try {
            ...
            if (parameters != null) {
                return namedJdbcTemplate.queryForLong(sql, parameters);
            } else {
     ...
     * Assert.hasText(sql,"sql语句不正确!"); if(bean!=null){ return jdbcTemplate.update(sql,
     * paramBeanMapper(bean)); }else{ return jdbcTemplate.update(sql); } }
     */

    /*
     * 批量处理操作 例如：update t_actor set first_name = :firstName, last_name = :lastName where id = :id 参数用冒号
     */
