...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
    ...
    public boolean loginUserIsHasMenuAuth(String requestPath, String clickFunctionId, String userid, String orgId) {
        ...
        String sql = "SELECT count(*) FROM t_s_function f,t_s_role_function  rf,t_s_role_user ru " + " WHERE f.id=rf.functionid AND rf.roleid=ru.roleid AND "
        ...
        if (authSize <= 0) {
            ...
            String functionOfOrgSql = "SELECT count(*) from t_s_function f, t_s_role_function rf, t_s_role_org ro  " + "WHERE f.ID=rf.functionid AND rf.roleid=ro.role_id "
    ...
    /**
     * 【AuthInterceptor】获取登录用户的页面控件权限（表单权限、按钮权限） {逻辑说明： 查询菜单的页面控件权限，排除授权用户的页面控件权限，剩下未授权的页面控件权限}
     * 
     * @param userId 用户ID
