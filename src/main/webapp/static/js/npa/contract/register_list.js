  ...
  if (rec.isSubmit == '1') {
    if (rec.stat == 'e' || rec.stat == 't') {
      return '<a href="#" class="ace_button" alt="查看回款记录" onclick="recallList(' + index + ')" style="background-color:#9e123e;"><i class="fa fa-eye"></i>查看回款记录</a>';
            ...
            + ')" style="background-color:#2f3c45e6;"><i class="fa fa-balance-scale"></i>收回款期初值设置</a> ' + '<a href="#" class="ace_button" alt="借冒名设置" onclick="impersonate('
            + index + ')" style="background-color:#e3402d;"><i class="fa fa-frown-o"></i>借冒名设置</a>'; }
  } else {
