...
// add--start--Author:xugj date:20160531 for: TASK #1089
// 【demo】针对jeecgdemo，实现一个新的页面方式
/**
 * 更新跳转新页面
 * 
 * @param title
...
// add--end--Author:xugj date:20160531 for: TASK #1089
// 【demo】针对jeecgdemo，实现一个新的页面方式

// add--start--Author:gengjiajia date:20160802 for: TASK #1175
// 批量添加数据的时popup多值的传递
// popup
// object: pobj当前操作的文本框.
// tablefield:对应字典TEXT,要从popup报表中获取的字段.inputnames:对应字典CODE,当前需要回填数据的文本框名称.
// pcode:动态报表的code
/**
 * object: pobj当前操作的文本框. tablefield:对应字典TEXT,要从popup报表中获取的字段.
 * inputnames:对应字典CODE,当前需要回填数据的文本框名称. pcode:动态报表的code
 */
					    	...
					    	for(var i1=0;i1<fields.length;i1++){
							   var str = "";
						    	...
						    	if(str.indexOf("undefined")==-1){
						    		if(inputs.length>1){
						    		﻿	// update--begin--author:scott date:20171031 for:TASK
                      // #2385 online和代码生成器 一对多行popup多字段赋值问题解决-----------
...
function initI18nConfig() {
    ...
    $.i18n.properties({
        ...
        path:basePath + '/plug-in/i18n/',   // 注意这里路径是你属性文件的所在文件夹
        mode:'map',
        language:i18n_browser_Lang,// 这就是国家代号 name+language刚好组成属性文件名：strings+zh
 ...
 * @returns
 */
function createDialogWithCallback(title, content, url, data, callbackFuncation, noShade) {
  $.dialog.setting.zIndex = getzIndex(true);
  var navigatorName = "Microsoft Internet Explorer"; 
  if( navigator.appName == navigatorName ||"default,shortcut".indexOf(getCookie("JEECGINDEXSTYLE"))>=0){ 
    $.dialog.confirm(content, function(){
      doSubmitWithCallBack(url, data, callbackFuncation);
    }, function(){
    });
  }else{
    layer.open({
 ...
 * @returns
 */
function doSubmitWithCallBack(url, data, callbackFuncation) {
  var paramsData = data;
  $.ajax({
    async : false,
    cache : false,
 ...
 * @returns
 */
function createWindowWithCallBack(title, url ,width,height, callbackFunction) {
  if(width=="100%" || height=="100%"){
    width = window.top.document.body.offsetWidth;
    height =window.top.document.body.offsetHeight-100;
  }else{
