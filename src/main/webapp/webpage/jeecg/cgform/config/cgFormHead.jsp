...
<body style="overflow-y: hidden; overflow-x: hidden;" scroll="no">
	...
	<t:formvalid formid="formobj" dialog="true" usePlugin="password" beforeSubmit="beforeSub();" layout="table" tiptype="1"
		...
		<table cellpadding="0" cellspacing="1" class="formtable">
			<tr>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="table.name" />
						:
					</label></td>
				<td class="value"><input class="inputxt" id="tableName" name="tableName" value="${cgFormHeadPage.tableName}"
					prefixName="" <c:if test="${not empty cgFormHeadPage.tableName}">readonly="readonly"</c:if> datatype="*"
					validType="cgform_head,table_Name,id" nullmsg=<t:mutiLang langKey="please.input.table.name"/>> <span
						class="Validform_checktip"></span></td>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="table.description" />
						:
					</label></td>
				<td class="value"><input class="inputxt" id="content" name="content" value="${cgFormHeadPage.content}"
					datatype="s2-100" nullmsg=<t:mutiLang langKey="please.input.table.content"/>> <span
						class="Validform_checktip"></span></td>
			</tr>
			<tr>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="pk.strategies" />
						:
					</label></td>
				<td class="value" id="jformPkTypeTd" <c:if test="${cgFormHeadPage.jformPkType ne 'SEQUENCE'}">colspan="3"</c:if>>
					<select id="jformPkType" name="jformPkType" onchange="jformPkTypeChange();">
						<option value="UUID" <c:if test="${cgFormHeadPage.jformPkType eq 'UUID'}"> selected='selected'</c:if>><t:mutiLang
								langKey="common.uuid36bit" /></option>
						<option value="NATIVE" <c:if test="${cgFormHeadPage.jformPkType eq 'NATIVE'}"> selected='selected'</c:if>><t:mutiLang
								langKey="common.native.auto.increment" /></option>
						<option value="SEQUENCE" <c:if test="${cgFormHeadPage.jformPkType eq 'SEQUENCE'}"> selected='selected'</c:if>><t:mutiLang
				...
				<td class="value" id="jformPkSequenceV"
					...
					id="jformPkSequence" name="jformPkSequence" type="text" class="inputxt" value="${cgFormHeadPage.jformPkSequence}" />
			...
			<tr>
				...
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="form.category" />
						:
					</label></td>
				...
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="table.type" />
						:
					</label></td>
				<td class="value"><select id="jformType" name="jformType" onchange="formTypeChange();">
						<option value="1" <c:if test="${cgFormHeadPage.jformType eq '1'}"> selected='selected'</c:if>><t:mutiLang
								langKey="single.table" /></option>
						<option value="2" <c:if test="${cgFormHeadPage.jformType eq '2'}"> selected="selected"</c:if>><t:mutiLang
								langKey="master.table" /></option>
						<option value="3" <c:if test="${cgFormHeadPage.jformType eq '3'}"> selected="selected"</c:if>><t:mutiLang
					...
					<div id="relation_type_div" style="display: none;">
						...
						<t:mutiLang langKey="sequence.name" />
						:<input class="inputxt" style="width: 30px" id="tabOrder" name="tabOrder" value="${cgFormHeadPage.tabOrder}"
			...
			<tr>
				...
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="form.template.style_pc" />
						:
					</label></td>
				...
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="form.template.style_mobile" />
						:
					</label></td>
				<td class="value"><select id="formTemplateMobile" name="formTemplateMobile"
					temVal="${cgFormHeadPage.formTemplateMobile}"></select></td>
				<!--add-start--Author:scott  Date:20160301 for：online表单移动样式单独配置-->
			</tr>
			<tr>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="show.checkbox" />
						:
					</label></td>
				<td class="value"><select id="isCheckbox" name="isCheckbox">
						<option value="N" <c:if test="${cgFormHeadPage.isCheckbox eq 'N'}"> selected="selected"</c:if>><t:mutiLang
								langKey="common.no" /></option>
						<option value="Y" <c:if test="${cgFormHeadPage.isCheckbox eq 'Y'}"> selected="selected"</c:if>><t:mutiLang
								langKey="common.yes" /></option>
				</select></td>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="is.page" />
						:
					</label></td>
				<td class="value"><select id="isPagination" name="isPagination">
						<option value="Y" <c:if test="${cgFormHeadPage.isPagination eq 'Y'}"> selected='selected'</c:if>><t:mutiLang
								langKey="common.yes" /></option>
						<option value="N" <c:if test="${cgFormHeadPage.isPagination eq 'N'}"> selected="selected"</c:if>><t:mutiLang
								langKey="common.no" /></option>
				</select></td>
			</tr>
			<tr>
				<td align="right"><label class="Validform_label">
						<t:mutiLang langKey="is.tree" />
						:
					</label></td>
				...
				<td class="value"><select id="querymode" name="querymode">
						<option value="single" <c:if test="${cgFormHeadPage.querymode eq 'single'}"> selected="selected"</c:if>><t:mutiLang
								langKey="single.query" /></option>
						<option value="group" <c:if test="${cgFormHeadPage.querymode eq 'group'}"> selected="selected"</c:if>><t:mutiLang
			...
			<c:if test="${cgFormHeadPage.jformType eq '2'}">
				<tr id="fb_tb">
					<td align="right"><label class="Validform_label">
							<t:mutiLang langKey="slave.table" />
						</label></td>
			...
			<tr class="tree">
				...
				<td class="value"><input class="inputxt" id="treeParentIdFieldName" name="treeParentIdFieldName"
					value="${cgFormHeadPage.treeParentIdFieldName}" datatype="s2-100"> <span class="Validform_checktip"></span></td>
				...
				<td class="value"><input class="inputxt" id="treeFieldname" name="treeFieldname"
					value="${cgFormHeadPage.treeFieldname}" datatype="s2-100"> <span class="Validform_checktip"></span></td>
			</tr>
		...
		<div id="tabs" class="easyui-tabs" tabPosition="top" fit="false"
			...
			<div title='<t:mutiLang langKey="database.property"/>' width="auto"
				...
				<c:if test="${cgFormHeadPage.tableType != '1'}">
					<div style="height: 25px;" class="datagrid-toolbar">
						<a id="addColumnBtn" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="addColumnBtnClick();"
							href="#"><t:mutiLang langKey="common.add.to" /></a> <a id="delColumnBtn" class="easyui-linkbutton"
							data-options="iconCls:'icon-remove'" onclick="delColumnBtnClick();" href="#"><t:mutiLang
			...
			<div title='<t:mutiLang langKey="common.index"/>' style="overflow: hidden;">
				<div style="height: 25px;" class="datagrid-toolbar">
					<a id="addColumnBtn1" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="addIndexBtnClick();"
						href="#"><t:mutiLang langKey="common.add.to" /></a> <a id="delColumnBtn1" class="easyui-linkbutton"
						data-options="iconCls:'icon-remove'" onclick="delIndexBtnClick();" href="#"><t:mutiLang
							langKey="common.delete" /></a>
				</div>
