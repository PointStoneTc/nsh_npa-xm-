	...
	<table id="jformOrderTicket2_table" class="table table-bordered table-hover" style="margin-bottom: 0;">
		...
		<tbody id="add_jformOrderTicket2_table">
			<c:if test="${fn:length(jformOrderTicket2List)  <= 0 }">
				<tr>
					<td><input style="width: 20px;" type="checkbox" name="ck" /> <input name="jformOrderTicket2List[0].id"
						type="hidden" /> <input name="jformOrderTicket2List[0].fckId" type="hidden" /></td>

					<td scope="row">
						<div name="xh"></div>
					</td>
					<td><input name="jformOrderTicket2List[0].ticketCode" maxlength="100" type="text" class="form-control"
						style="width: 120px;" datatype="*" ignore="checked" /></td>
					<td><input name="jformOrderTicket2List[0].tickectDate" maxlength="10" type="text" class="form-control"
				...
				<c:forEach items="${jformOrderTicket2List}" var="poVal" varStatus="stuts">
					<tr>
						<td><input style="width: 20px;" type="checkbox" name="ck" /> <input
							name="jformOrderTicket2List[${stuts.index }].id" type="hidden" value="${poVal.id }" /> <input
							name="jformOrderTicket2List[${stuts.index }].fckId" type="hidden" value="${poVal.fckId }" /></td>

						<td scope="row">
							<div name="xh">${stuts.index+1 }</div>
						</td>

						<td><input name="jformOrderTicket2List[${stuts.index }].ticketCode" maxlength="100" type="text"
							class="form-control" style="width: 120px;" datatype="*" ignore="checked" value="${poVal.ticketCode }" /></td>
						<td><input name="jformOrderTicket2List[${stuts.index }].tickectDate" maxlength="10" type="text"
							class="form-control" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"
							style="background: url('plug-in/ace/images/datetime.png') no-repeat scroll right center transparent; width: 120px;"
							datatype="*" ignore="checked"
							value="<fmt:formatDate value='${poVal.tickectDate}' type="date" pattern="yyyy-MM-dd hh:mm:ss"/>" /></td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
