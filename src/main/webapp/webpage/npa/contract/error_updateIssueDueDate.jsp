		...
		<table style="width: 100%;" cellpadding="0" cellspacing="1" class="viewtable">
			<tbody>
				<tr>
					...
					<td><input type="text" class="Wdate" onClick="WdatePicker({isShowClear:false,readOnly:true,minDate:'#F{$dp.$D(\'dueDate\')}'})" name="issueDate" id="issueDate"
						value="${issueDate }" datatype="*" sucmsg="贷款发放日验证通过!" nullmsg="请输入贷款发放日!" /></td>
				</tr>
				<tr>
					<th><label class="required_sign">贷款到期日</label></th>
					<td><input type="text" class="Wdate" onClick="WdatePicker({isShowClear:false,readOnly:true,minDate:'#F{$dp.$D(\'issueDate\')}'})" name="dueDate" id="dueDate"
						value="${dueDate }" datatype="*" sucmsg="贷款到期日验证通过!" nullmsg="请输入贷款到期日!" /></td>
				</tr>
			</tbody>
		</table>
