...
<script type="text/javascript"
	src="${webRoot}/static/js/npa/contract/recall_view.js"></script>
...
<body class="easyui-layout">
	<div id="htxq"
		data-options="region:'west', href:'register.do?toDetail&id=${view.loanContract.id }'"
		title="合同详情" style="width: 52%; padding: 5px"></div>
		...
		<div class="easyui-tabs" data-options="fit:true"
			...
			<div title="利息计算" style="padding: 10px">
				<form name="formobj" id="formobj" method="post"
					action="${webRoot}/recoveryMoney.do?saveOrUpdate">
					<input type="hidden" name="id" id="id" value="" /> <input
						type="hidden" name="loanContracId" id="loanContracId"
						value="${view.loanContract.id }" /> <input type="hidden"
						name="interestBearingPrincipal" id="interestBearingPrincipal"
						value="" /> <input type="hidden" name="recoveryInterest"
						id="recoveryInterest" value="" />
					<h3 class="lrq">试算</h3>
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="Wdate"
									...
									name="recoveryDate" id="recoveryDate" datatype="*"
									sucmsg="收回时间验证通过!" nullmsg="请输入收回时间!" readonly="readonly" /></td>
								<th><label class="required_sign">收回人</label></th>
								<td><input type="text" name="recoveryPerson"
									id="recoveryPerson" maxlength="15" datatype="*"
									sucmsg="收回人验证通过!" nullmsg="请输入收回人!" /></td>
							</tr>
							<tr>
								<th><label class="required_sign">收回本金</label></th>
								<td><input type="text" name="recoveryPrincipal"
									id="recoveryPrincipal" value="0.0" maxlength="11"
								...
								<td><input type="text" name="interestRate"
									id="interestRate" value="${view.loanContract.interestRate }"
									maxlength="6" datatype="/^[1-9][0-9]?(\.\d{1,3})?$/"
							...
							<tr>
								...
								<td><input type="text" class="Wdate"
									...
									name="interestDate" id="interestDate" value="" datatype="*"
									sucmsg="起息日验证通过!" nullmsg="请输入起息日!" /></td>
								<th><label>挂息金额</label></th>
								<td><input type="text" name="hangInterest"
									id="hangInterest" value="0.0" maxlength="11"
									datatype="/^(0|(0{1}\.\d{1,2}|([1-9]{1}\d{0,6}\.{1}\d{1,2})|([1-9]{1}\d{0,6})))$/"
									sucmsg="挂息金额验证通过!" nullmsg="请输入挂息金额!" /></td>
							</tr>
							<tr>
								<th><label class="required_sign">收回方式</label></th>
								<td colspan="3"><select name="recoveryMode"
									id="recoveryMode" datatype="*" nullmsg="请选择收回方式!">
					...
					<table style="width: 100%;" cellpadding="0" cellspacing="1"
						...
						<tbody>
							<tr>
								...
								<td><input type="text" id="b2" readonly="readonly"
									value="0.0" datatype="custom2" sucmsg="剩余本金验证通过!"
					...
					<table id="lixika" class="easyui-datagrid"
						data-options="rownumbers:true,singleSelect:true,showFooter:true,striped:true"
						style="width: 100%;">
						<thead>
							<tr>
					...
					<div style="padding: 5px 0; text-align: right">
						<a href="#" class="easyui-linkbutton" onclick="calculator()">计算</a>
						<a href="#" class="easyui-linkbutton" onclick="allRecall()">全额还款</a>
						<a href="#" class="easyui-linkbutton" id="btn_sub">保存</a>
					</div>
				</form>
			</div>
