...
<body style="width: 99%;">
	<form name="formobj" id="formobj" method="post" action="register.do?saveOrUpdate">
		<!-- 借款信息 -->
		<div>
			<input type="hidden" name="loanContract.id" id="ctid" value="${view.loanContract.id }" /> <input type="hidden"
				name="loanContract.corporateOrgName" value="${view.loanContract.corporateOrgName }" />
			...
			<table style="width: 100%;" cellpadding="0" cellspacing="1" class="viewtable">
				<tbody>
					<tr>
						...
						<th>
							<label class="required_sign">贷款金额</label>
						</th>
						<td><input type="text" name="loanContract.amount"
							value="<fmt:formatNumber pattern="0" maxFractionDigits="0" value="${view.loanContract.amount }" />" maxlength="7"
						...
						<td><input type="text" class="Wdate"
							...
							name="loanContract.dueDate" id="dueDate" value="${view.loanContract.dueDate }" datatype="*" sucmsg="贷款到期日验证通过!"
							nullmsg="请输入贷款到期日!" /></td>
					</tr>
					<tr>
						<th>
						...
						<td><input type="text" name="loanContract.interestRate"
							...
							maxlength="6" tip="千分比格式" altercss="gray" datatype="/^[1-9][0-9]?(\.\d{1,3})?$/" sucmsg="利率验证通过!"
							nullmsg="请输入利率!" /></td>
						<th>
							<label class="required_sign">信贷员</label>
						</th>
						<td><input type="text" name="loanContract.officer" value="${view.loanContract.officer }" maxlength="15"
							datatype="*" sucmsg="信贷员验证通过!" nullmsg="请输入信贷员!" /></td>
						<th>
							<label class="required_sign">所属支行</label>
						</th>
					...
					<tr>
						...
						<td><select name="loanContract.litigationStat" id="loanContract.litigationStat" datatype="*"
							nullmsg="请选择诉讼状态!">
								<option value="" selected="selected">---请选择---</option>
					...
					<tr>
						...
						<td colspan="7"><input type="text" name="loanContract.remark" value="${view.loanContract.remark }"
							maxlength="100" style="width: 365px;" /></td>
					</tr>
				</tbody>
			</table>
			...
			<table style="width: 100%;" cellpadding="0" cellspacing="1" class="viewtable">
				...
				<tbody>
					<tr>
						...
						<td><input type="text" name="borrower.name" value="${view.borrower.name}" maxlength="50" datatype="*"
							sucmsg="姓名验证通过!" nullmsg="请输入姓名!" /></td>
						<th>
							<label class="required_sign">身份证号</label>
						</th>
						<td><input type="text" name="borrower.idNumber" id="borrowerIdNumber" value="${view.borrower.idNumber }"
					...
					<tr>
						...
						<td><input type="radio" name="borrower.sex" value="m" onclick="return false;" readonly="readonly">男<input
							type="radio" name="borrower.sex" value="f" onclick="return false;" readonly="readonly">女</td>
						<th>
							<label>手机号码</label>
						</th>
						<td><input type="text" name="borrower.phone" value="${view.borrower.phone }" maxlength="11" datatype="m"
						...
						<td><input type="radio" name="loanContract.isImpersonate" value="1">是<input type="radio"
							name="loanContract.isImpersonate" value="0" checked="checked">否</td>
					</tr>
				</tbody>
			</table>
			<div style="margin-top: 5px;">
				<a id="borrower_search" class="easyui-linkbutton" data-options="iconCls:'icon-search'" attr1="borrower_table">查询</a><a
					id="borrower_clean" class="easyui-linkbutton" data-options="iconCls:'icon-le-back'" attr1="borrower_table">清空</a>
			</div>
			<div class="split blue_line"></div>
		</div>
	...
	<div id="guarantee_template" style="display: none">
		<div style="position: relative;" id="" name="guarantee_table_">
			<input type="hidden" name="loanContractGuarantees[-].id" value="" /> <input type="hidden"
				name="loanContractGuarantees[-].loanContracId" value="" /> <input type="hidden"
				name="loanContractGuarantees[-].guaranteeId" value="" /> <input type="hidden"
				name="loanContractGuarantees[-].guarantee.id" value="" />
			<table style="width: 100%;" cellpadding="0" cellspacing="1" class="viewtable">
				<tbody>
					<tr>
						...
						<td><input type="text" name="loanContractGuarantees[-].guarantee.name" value="" maxlength="50" datatype="*"
							sucmsg="姓名验证通过!" nullmsg="请输入姓名!" /></td>
						<th>
							<label class="required_sign">身份证号</label>
						</th>
						<td><input type="text" name="loanContractGuarantees[-].guarantee.idNumber" value="" maxlength="18"
							datatype="*" sucmsg="身份证号验证通过!" nullmsg="请输入身份证号!" /></td>
						<th>
							<label>出生年月</label>
						</th>
					...
					<tr>
						...
						<td><input type="radio" name="loanContractGuarantees[-].guarantee.sex" value="m" onclick="return false;"
							readonly="readonly">男<input type="radio" name="loanContractGuarantees[-].guarantee.sex" value="f"
							onclick="return false;" readonly="readonly">女</td>
						<th>
							<label>手机号码</label>
						</th>
						<td><input type="text" name="loanContractGuarantees[-].guarantee.phone" value="" maxlength="11" datatype="m"
							ignore="ignore" sucmsg="手机号码验证通过!" nullmsg="请输入手机号码!" /></td>
						<th>
							<label>通讯地址</label>
						</th>
						...
						<td><input type="radio" name="loanContractGuarantees[-].isImpersonate" value="1">是<input type="radio"
							name="loanContractGuarantees[-].isImpersonate" value="0" checked="checked">否</td>
					</tr>
				</tbody>
			</table>
			<div style="margin-top: 5px;">
				<a name="person_search_btn" class="easyui-linkbutton" data-options="iconCls:'icon-search'" attr1="guarantee_table_">查询</a><a
					name="person_clean_btn" class="easyui-linkbutton" data-options="iconCls:'icon-le-back'" attr1="guarantee_table_">清空</a><a
					name="person_remove_btn" class="easyui-linkbutton" data-options="iconCls:'icon-remove'" attr1="guarantee_table_">删除</a>
			</div>
