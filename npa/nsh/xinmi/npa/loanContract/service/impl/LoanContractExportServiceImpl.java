  ...
  public String all(String fileSavePath) {
    StringBuilder sql = new StringBuilder(
        "select c.id, c.num_code, c.customer_no, c.lou_no, c.borrower_name, p.id_number,if(c.is_impersonate = 0, '否', '是') is_impersonate, c.amount, c.issue_date, c.due_date, c.interest_rate, ") //
            ...
            .append("c.stat, c.interest_date, c.recovery_principal, c.recovery_interest, c.hang_interes, t.guarantee_name")
            .append(" from npa_loan_contract c left join npa_natural_person p on c.borrower_id = p.id") //
            .append(
                " LEFT JOIN  (select  loan_contrac_id , GROUP_CONCAT(g.guarantee_name) guarantee_name from npa_loan_contract_guarantee g group  by g.loan_contrac_id) t ON c.id = t.loan_contrac_id ");
    ...
    // 创建xls
    String[] titles = new String[] {"序号", "档案编号", "客户号", "借据号", "借款人姓名", "借款人身份证号", "是否借冒名", "担保人", "贷款金额(元)", "贷款发放日", "贷款到期日", "利率(‰)", "不良形成日期", "信贷员", "所属支行", "诉讼状态", "担保方式",
        "处理方式", "起息日", "收回本金(元)", "收回利息(元)", "挂息(元)"};
    String[] vals = new String[] {"num_code", "customer_no", "c.lou_no", "borrower_name", "id_number", "is_impersonate", "guarantee_name", "amount", "issue_date", "due_date",
        "interest_rate", "unhealthy_date", "officer", "corporate_org_name", "litigation_stat", "guarantee_mode", "dispose_mode", "interest_date", "recoveryPrincipal",
        "recovery_interest", "hang_interes"};
