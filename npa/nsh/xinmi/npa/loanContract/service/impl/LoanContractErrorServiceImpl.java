...
public class LoanContractErrorServiceImpl extends CommonServiceImpl implements LoanContractErrorServiceI {
  ...
  public void getLoanContractRegisterList(LoanContractRegisterView loanContractRegisterView, String errorType, DataGrid dataGrid) {
    StringBuffer sql = new StringBuffer(
        "select c.id, c.num_code, c.customer_no, c.lou_no, c.borrower_name, p.id borrower_id, p.id_number, p.address, c.amount, c.issue_date, c.due_date, c.interest_rate, c.officer, c.corporate_org_name, c.litigation_stat, c.guarantee_mode, c.dispose_mode, c.recovery_principal, c.interest_date, c.recovery_interest, c.hang_interes ")
    ...
    for (Map<String, Object> rsMap : rs) {
      ...
      BeanUtil.setProperty(lcv, "borrowerId", rsMap.get("borrower_id"));
      BeanUtil.setProperty(lcv, "address", rsMap.get("address"));
      list.add(lcv);
    ...
    if (list.size() > 0) {
      ...
      sql = new StringBuffer(
          "select g.id, g.guarantee_id, g.loan_contrac_id, g.guarantee_name, p.id_number, p.address from npa_loan_contract_guarantee g left join npa_natural_person p on g.guarantee_id = p.id where g.loan_contrac_id in(");
      ...
      for (Map<String, Object> rsMap : rsgt) {
        ...
        BeanUtil.setProperty(guarantee, "address", rsMap.get("address"));
  ...
  @Override
  public boolean updateBorrower(String ids, String name, String sex, Date birthday, String idNumber, String address) throws Exception {
    // 判断身份证号是否存在,如果存在使用存在的身份号,否则新建
    if (oConvertUtils.isEmpty(idNumber)) {
      idNumber = "111111111111111111";
    }

    NaturalPerson person = naturalPersonService.isExist(name, idNumber);
    Long borrower_id;
    if (person == null) {
      NaturalPerson temp = new NaturalPerson(name, sex, birthday, idNumber, address, "1", "0");
      temp.setIsDelete("0");
      ...
      naturalPersonService.updateEntitie(person);
    }
  ...
  @Override
  public boolean updateGuarantee(String id, String guaranteesNames, String guaranteesIdNumbers, String guaranteesAddresses) throws Exception {
    // 1.先删除原来的数据
    ...
    String[] addresses = guaranteesAddresses.split(",");

    for (int x = 0; x < names.length; x++) {
      NaturalPerson person = naturalPersonService.isExist(names[x], idNumbers[x]);
      Long guarantee_id;
      String name;
      if (person == null) {
        NaturalPerson temp = new NaturalPerson(names[x], null, null, idNumbers[x], addresses[x], "0", "1");
        temp.setIsDelete("0");
        naturalPersonService.save(temp);
  ...
  public boolean commit(String ids) {
    TSUser loginUser = ResourceUtil.getSessionUser();
    StringBuffer sql = new StringBuffer("update npa_loan_contract_error set is_submit = 1, update_by = ?, update_date = NOW() where loan_contract_id in (").append(ids).append(")");
